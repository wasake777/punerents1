...
// the PMR: parks, stadiums, golf courses, nature reserves, recreation grounds,
// forest, quarries, military land, prisons, power stations, railway land and
// airport grounds. Pins can't be dropped there.
//
...
//
// For the same reason, `pitch`, `playground` and `garden` are NOT blocked
// either. They're small polygons (~600 of them) mostly sitting *inside*
// residential society compounds — a society's own cricket pitch or garden
// would otherwise mark a ~110m cell that also covers real flats. The
// categories kept below are all large standalone land with no housing on it.
//
// Each category below was checked against OSM's mapped residential land for
// the PMR before being added. Deliberately excluded for overlapping real
// housing: natural=wood (881 km² — a quarter of the bbox), landuse=industrial
// (MIDC estates contain PGs and hostels), natural=scrub, landuse=construction
// (sites become flats) and man_made=wastewater_plant. historic=fort was tried
// and reverted: it was worth only 6 cells, and Shaniwar Wada's cells cover the
// dense Peth lanes around it, which OSM does not tag as residential land.
//
// Caveat on landuse=military: it covers 59.5 km² of Southern Command, Khadki,
// Dehu Road and NDA land, but OSM draws cantonments as a single polygon, so a
// few civilian pockets inside them (Khadki bazaar) get blocked too. Accepted
// as a known trade-off — the alternative leaves 59 km² of bases pinnable.
const SELECTORS = [
  `["leisure"~"^(park|stadium|golf_course|recreation_ground|nature_reserve)$"]`,
  `["landuse"~"^(recreation_ground|village_green|railway|forest|quarry|military)$"]`,
  `["natural"="beach"]`,
  `["aeroway"="aerodrome"]`,
...
// OSM elements that are mistagged upstream and would block housing if used.
// Re-check these if the Overpass cache is ever refreshed — they may be fixed.
const EXCLUDE = new Set([
  "way/229849148", // Loyola High School, Pashan — tagged amenity=prison in OSM
                   // (wikidata Q6694572 confirms it's a school). Schools are
                   // deliberately pinnable, so this one must not mark cells.
]);
...
for (const el of elements) {
  if (EXCLUDE.has(`${el.type}/${el.id}`)) continue;
...
// Overpass polygons (parks, stadiums, golf courses, forest, quarries, military
// land, prisons, power stations, railway and airport land). One bit per ~110m
// cell; 1 = public place.
